...
all_losses = [Work_m, PDE1_m, PDE2_m, EQ1_m, EQ2_m, EQ3_m,
              BC6_m, BC8_m, BC10_m,
              BC12_m, BC14_m, BC16_m, BC18_m, BC20_m,
              Work_t, PDE1_t, EQ1_t, EQ2_t, BC1_t, BC2_t, BC5_t, BC6_t, BC7_t, BC8_t]

model = sn.SciModel([x, y, E, K], all_losses)
input_data = [x1, y1, e1, k1]
target_data = [np.zeros_like(x1)] * len(all_losses)

import sciann
print(sciann.__version__)


# --- PHASE 1: Warm-Start com ADAM ---
print("--- PHASE 1: Warm-Start (ADAM) - 10000 epochs ---")

history_adam = model.train(
    input_data,
    target_data,
    epochs=10000,
    batch_size=1000,
...
# --- PHASE 2: Fine-Tuning com L-BFGS-B ---
print("--- PHASE 2: Fine-Tuning (L-BFGS-B)")

history_lbfgs = model.train(
    input_data,
    target_data,
    epochs=1000,
    batch_size=1000,
...
print("Training concluded with optimized strategy (Adam + L-BFGS-B).")
...
itter = 0 
for word, loss in history_lbfgs.history.items():
...
fig = plt.figure(figsize=(12, 7))
plt.semilogy(history_lbfgs.history['loss'], label='Total_loss')
plt.legend()
plt.xlabel('epochs')
...
all_losses = pd.DataFrame(history_lbfgs.history)
itter = 0 
for word, loss in history_lbfgs.history.items():
    if word.endswith("loss"):
